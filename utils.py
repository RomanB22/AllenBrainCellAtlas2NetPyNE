            ...
            for id in specimen_model_id:
                ...
                if not os.path.isdir(workingDirPeri):
                    try:
                        ...
                        #os.rename(workingDirPeri+'/{:d}_fit.json'.format(id), workingDirPeri+'/config.json')
                    except:
                        NotLoadedPeri.append([workingDirPeri])
                        print("No data available for {}".format(workingDirPeri))
                if not os.path.isdir(workingDirAllActive):
                    try:
                        bp.cache_data(resActive, working_directory=workingDirAllActive)
                        #os.rename(workingDirAllActive+'/{:d}_fit.json'.format(id), workingDirAllActive+'/config.json')
                    except:
                        NotLoadedAllActive.append([workingDirAllActive])
                        print("No data available for {}".format(workingDirAllActive))
...
def convert2NetPyNE(workingDir, cellType, region, id, cellModel='Peri', SaveCell=True):
    ...
    cellRule['secLists']['soma'] = [sec for sec in cellRule.secs if ('soma' in sec)]  # soma
    if SaveCell:
        netParams.saveCellParamsRule(label=cellType+'_'+region+'_'+id, fileName=cwd+'/cells/%s_%s_cellParams.pkl' % (cellType+'_'+region+'_'+id, cellModel))
